...
class DataAggregator:
    ...
    def aggregate_bands_by_average(self, input_dataset_path: Path, output_folder_path: Path) -> Path:
        ...
        count_data = np.zeros((output_dataset.RasterYSize, output_dataset.RasterXSize), dtype=np.int64)
    ...
    def create_aggregated_dataset_and_band(self, input_dataset: gdal.Dataset, output_path: Path, output_file_name_without_extension: str) -> tuple:
        ...
        output_dataset.SetGeoTransform(self.get_geotransform(input_dataset, has_sub_datasets))
        output_dataset.SetProjection(self.get_projection(input_dataset, has_sub_datasets))
    ...
    def aggregate_files_by_average(self, input_datasets_paths: list, output_folder_path: Path) -> Path:
        ...
        no_data_value = output_band.GetNoDataValue()
        sum_data = np.zeros((output_dataset.RasterYSize, output_dataset.RasterXSize), dtype=np.float32)
        count_data = np.zeros((output_dataset.RasterYSize, output_dataset.RasterXSize), dtype=np.int32)
